...
def getSessionId() -> int:
    # lastSessionId = Gilfoyle.query.order_by(desc(Gilfoyle.id)).limit(1).first()
...
def collectData() -> Gilfoyle:
    # data = Gilfoyle(timestamp=getTime(), ram=getRAM(), battery=getBatteryLevel(), temperature=getTemp(), rpm=getFanRPM())
    data = Gilfoyle(
        sessionId=sessionId,
